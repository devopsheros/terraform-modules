...
module "gke" {
  ...
  kms_key_ring_name = "${var.env_name}-clustr-kms"
...
resource "google_compute_network_peering" "peer" {
  ...
  name         = "monitoring-to-${var.peering_networks[count.index]}"
  network      = module.vpc.network
  peer_network = "projects/${var.gcp_project}/global/networks/${var.peering_networks[count.index]}-vpc"
}

// PROMETHEUS CLOUD DNS
resource "google_dns_record_set" "a_record_subdomain_prom_qa" {
...
resource "google_dns_record_set" "cname_record_subdomain_prom_qa" {
  name = "www.prom-qa.devopsheros.com."
  type = "CNAME"
  ttl = 300
  managed_zone = "dns-zone"
  rrdatas = [
    google_dns_record_set.a_record_subdomain_prom_qa.name
  ]
}

resource "google_dns_record_set" "a_record_subdomain_prom_prod" {
...
resource "google_dns_record_set" "cname_record_subdomain_prom_prod" {
  name = "www.prom-prod.devopsheros.com."
  type = "CNAME"
  ttl = 300
  managed_zone = "dns-zone"
  rrdatas = [
    google_dns_record_set.a_record_subdomain_prom_prod.name
  ]
}
